'''*
    Derived from: https://github.com/coutyou/DMRVisNet/blob/main/src/dataset/FoggyDataset.py
    Original Copyright (c) 2021 CoutYou
    License: MIT License
    Full license: https://github.com/coutyou/DMRVisNet/blob/main/LICENSE
    The following code is used under the terms of the MIT License.
'''
import os
import pickle
import cv2
...
if __name__ == "__main__":
    root_path = "/media/arthurthomas/CN300/vis/FoggyAirsimCityImages/data_pkl/"
    val_dataset = FACIDataset(root_path, phase='test')

    batch_size = 1
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=0, drop_last=True)
    for batch_idx, data in enumerate(val_loader):
        FoggyScene = data["FoggyScene_0.05"]
        DepthPerspective = data["DepthPerspective"]
        t = data["t_0.05"]
        Visibility = data["Visibility"]
        A = data["A"]
        Scene = data["Scene"]
        SkyMask = data["SkyMask"]
        
        FoggyScene = (FoggyScene.permute(0, 2, 3, 1)[0].numpy() * 255).astype(np.uint8)
        Scene = (Scene.permute(0, 2, 3, 1)[0].numpy() * 255).astype(np.uint8)
        t = (t.permute(0, 2, 3, 1)[0].numpy() * 255).astype(np.uint8)
        t = cv2.applyColorMap(t, cv2.COLORMAP_INFERNO)
        SkyMask = (SkyMask.permute(0, 2, 3, 1)[0].numpy() * 255).astype(np.uint8)
        SkyMask = cv2.applyColorMap(SkyMask, cv2.COLORMAP_INFERNO)
        FoggyScene = cv2.cvtColor(FoggyScene, cv2.COLOR_RGB2BGR)
        Scene = cv2.cvtColor(Scene, cv2.COLOR_RGB2BGR)
        cv2.namedWindow("FoggyScene",0)
        cv2.imshow("FoggyScene", FoggyScene)
        cv2.namedWindow("Scene",0)
        cv2.imshow("Scene", Scene)
        cv2.namedWindow("t",0)
        cv2.imshow("t", t)
        cv2.namedWindow("SkyMask",0)
